 ...
 *
 * <p /> The tool bar is "bound" to the loader using the
 * {@link #bind(PagingLoader)} method.
 */
public class SelectGridToolsBar extends ToolBar {
    ...
    private LoaderHandler<PagingLoadConfig, ?> handler = new LoaderHandler<PagingLoadConfig, PagingLoadResult<?>>() {
        ...
        public void onLoad(LoadEvent<PagingLoadConfig, PagingLoadResult<?>> event) {
            refresh.setIcon(appearance.refresh());
            setEnabled(savedEnableState);
            SelectGridToolsBar.this.onLoad(event);
            loading = false;
        }
     ...
     */
    @UiConstructor
    public SelectGridToolsBar(int pageSize) {
     ...
     * @param pageSize the page size
     */
    public SelectGridToolsBar(ToolBarAppearance toolBarAppearance, PagingToolBarAppearance appearance, int pageSize) {
