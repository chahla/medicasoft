...
import com.rsoft.medicasoft.shared.model.ProfessionPropertiesAccess;
...
public class ProfessionCView extends ViewGridBase<Profession> {
	...
	@Override
	public void remove() {
		List<Profession> Professions = grid.getSelectionModel()
				.getSelectedItems();
		for (Profession Profession : Professions) {
			if (Profession.getEntityId() != null) {
				presenter.addModelToBeRemoved(Profession);
				Profession foundModel = listStore
	...
	public void ignore() {
		List<Profession> Professions = grid.getSelectionModel()
				.getSelectedItems();
		for (Profession Profession : Professions) {
			Profession foundModel = listStore.findModelWithKey(listStore
					.getKeyProvider().getKey(Profession));
			if (foundModel != null) {
	...
	public void loadDatas() {
		try {
			RpcProxy<FilterPagingLoadConfig, PagingLoadResult<Profession>> proxy = new RpcProxy<FilterPagingLoadConfig, PagingLoadResult<Profession>>() {
				@Override
				public void load(FilterPagingLoadConfig loadConfig,
						AsyncCallback<PagingLoadResult<Profession>> callback) {
					listStore.clear();
					listStore.commitChanges();
        ...
        if (filters.getFilter("professionId") != null && filters.getFilter("professionId").getFilterConfig() != null && !filters.getFilter("professionId").getFilterConfig().isEmpty()){
          for(FilterConfig fc : filters.getFilter("professionId").getFilterConfig()) {
            if(fc.getValue() != null) {
              wrapper.addFilter(new XFilter(fc.getComparison(),"professionId", fc.getType(), fc.getValue()));
            }
          }
        }
        ...
        if (filters.getFilter("classification") != null && filters.getFilter("classification").getFilterConfig() != null && !filters.getFilter("classification").getFilterConfig().isEmpty()){
          for(FilterConfig fc : filters.getFilter("classification").getFilterConfig()) {
            if(fc.getValue() != null) {
              wrapper.addFilter(new XFilter(fc.getComparison(),"classification", fc.getType(), fc.getValue()));
            }
          }
        }
          ...
          for(FilterConfig fc : filters.getFilter("updatedOn").getFilterConfig()) {
            if(fc.getValue() != null) {
              wrapper.addFilter(new XFilter(fc.getComparison(),"updatedOn", fc.getType(), fc.getValue()));
					...
					UserRequestCallbackAdapter<Profession> urc = new UserRequestCallbackAdapter<Profession>();
					urc.setRequestDescriptor(new RequestDescriptor(ENTITY_NAME));
					urc.setAsyncCallback(callback);
					urc.setLoadConfig(loadConfig);
					presenter.getEventBus().executeProfession(ActionCommand.COLUMNAR_SEARCH, urc);
					showInfoBanner(!userHideBannerInfo);
				}
			};
			pagingLoader = new PagingLoader<FilterPagingLoadConfig, PagingLoadResult<Profession>>(
					proxy) {
			...
			pagingLoader
					.addLoadHandler(new LoadResultListStoreBinding<FilterPagingLoadConfig, Profession, PagingLoadResult<Profession>>(
							listStore));
	...
	protected void resetForm() {
		RpcProxy<FilterPagingLoadConfig, PagingLoadResult<Profession>> proxy = new RpcProxy<FilterPagingLoadConfig, PagingLoadResult<Profession>>() {
			...
			public void load(FilterPagingLoadConfig loadConfig,
				...
				UserRequestCallbackAdapter<Profession> urc = new UserRequestCallbackAdapter<Profession>() {
					@Override
					public void onSingleOperationSuccessed(Profession model) {
						hideProgressBar();
					}

					@Override
					public void onMultipleOperationsFailed(
							List<Profession> model, Throwable cause) {
						hideProgressBar();
					}
				};
				urc.setAsyncCallback(callback);
				...
				urc.setRequestDescriptor(new RequestDescriptor(ENTITY_NAME));
				presenter.getEventBus().executeProfession(
	...
	private PagingLoader<FilterPagingLoadConfig, PagingLoadResult<Profession>> pagingLoader = null;
	private RowNumberer<Profession> numberer;

	@Override
	public Widget asWidget() {
		ENTITY_NAME = "Profession";
		ProfessionPropertiesAccess propertiesAccess = GWT
				.create(ProfessionPropertiesAccess.class);
		listStore = new ListStore<Profession>(propertiesAccess.key());
		IdentityValueProvider<Profession> identity = new IdentityValueProvider<Profession>();
		...
		ColumnConfig<Profession, String> createdByColumn = new ColumnConfig<Profession, String>(
				propertiesAccess.createdBy(), 30, messages.createdBy());
		list.add(createdByColumn);
		ColumnConfig<Profession, Date> createdOnColumn = new ColumnConfig<Profession, Date>(
				propertiesAccess.createdOn(), 10, messages.createdOn());
		list.add(createdOnColumn);
		ColumnConfig<Profession, String> updatedByColumn = new ColumnConfig<Profession, String>(
		...
		final GridEditing<Profession> editing = new GridInlineEditing<Profession>(
				grid);
		editing.addBeforeStartEditHandler(new BeforeStartEditHandler<Profession>() {
			@Override
			public void onBeforeStartEdit(BeforeStartEditEvent<Profession> event) {
				editing.completeEditing();
				int row = event.getEditCell().getRow();
				Profession model = listStore.get(row);
				model.setUpdating(true);
			}
		});
		editing.addCompleteEditHandler(new CompleteEditHandler<Profession>() {
			@Override
			public void onCompleteEdit(CompleteEditEvent<Profession> event) {
				int row = event.getEditCell().getRow();
				Profession model = listStore.get(row);
      ...
      classificationField.addValidator(new MaxLengthValidator(50));      editing.addEditor(classificationColumn, classificationField);
		...
		toolsBar.setParamInoutSuffix("Profession");
		...
		filters = new GridFilters<Profession>();
		listStore.addStoreRecordChangeHandler(new StoreRecordChangeHandler<Profession>() {
			@Override
			public void onRecordChange(
					StoreRecordChangeEvent<Profession> event) {
				Profession changedModel = event.getRecord().getModel();
		...
		//Add Filters
        StringFilter<Profession> descriptionFilter = new StringFilter<Profession>(propertiesAccess.description());
        filters.addFilter(descriptionFilter);
        StringFilter<Profession> classificationFilter = new StringFilter<Profession>(propertiesAccess.classification());
        filters.addFilter(classificationFilter);
        StringFilter<Profession> createdByFilter = new StringFilter<Profession>(propertiesAccess.createdBy());
        filters.addFilter(createdByFilter);
        DateFilter<Profession> createdOnFilter = new DateFilter<Profession>(propertiesAccess.createdOn());
        filters.addFilter(createdOnFilter);
        StringFilter<Profession> updatedByFilter = new StringFilter<Profession>(propertiesAccess.updatedBy());
        filters.addFilter(updatedByFilter);
        DateFilter<Profession> updatedOnFilter = new DateFilter<Profession>(propertiesAccess.updatedOn());
		...
		grid.addCellClickHandler(new CellClickHandler() {
			...
			public void onCellClick(CellClickEvent event) {
				Profession model = grid.getSelectionModel().getSelectedItem();
				ViewUtils.showAuditInfos(htmlMessage, model.getGlobalAuditInfo());
			}
		});
		grid.getView().setViewConfig(new GridViewConfig<Profession>() {
			@Override
			public String getColStyle(Profession model,
					ValueProvider<? super Profession, ?> valueProvider,
					int rowIndex, int colIndex) {
				return null;
			}

			@Override
			public String getRowStyle(Profession model, int rowIndex) {
	...
	public void setPresenter(IPresenter p) {
		this.presenter = (ProfessionPresenter) p;
	}
	...
	@Override
	public void updateView(Profession model) {
	...
	@Override
	public void updateView(List<Profession> models) {
		try {
			if (models != null) {
				boolean allOperationsSuccess = true;
				for (Profession model : models) {
	...
	@Override
	public Profession updateModel(Profession model) {
		// TODO Auto-generated method stub
		return null;
	}
	...
	@Override
	public void onDataChanged(Profession model, boolean reloadDetails,
	...
	protected void execute(ActionCommand command,
			UserRequestCallbackAdapter<Profession> urc) {
		presenter.getEventBus().executeProfession(command, urc);
	}

	@Override
	protected void setModelsToBePersisted(ListStore<Profession> listStore) {
